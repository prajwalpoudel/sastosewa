<footer class="bg-zinc-800 text-white">
    <div class="content-container py-4">
        <div class="grid grid-cols-4">
            <div class="quick-links">
                <p class="text-xl underline underline-offset-2 py-2 ">Quick Links</p>
                <ul class="text-sm px-2 py-1">
                    <li class="py-1">
                        <a href="{{ url('design/tour') }}">Home</a>
                    </li>
                    <li class="py-1">
                        <a href="{{ url('design/tour') }}">About</a>
                    </li>
                    <li class="py-1">
                        <a href="{{ url('design/tour') }}">Contact Us</a>
                    </li>
                </ul>
            </div>
            <div class="services">
                <p class="text-xl underline underline-offset-2 py-2 text-white">Services</p>
                <ul class="text-sm px-2 py-1">
                    <li class="py-1">
                        <a href="{{ url('design/tour') }}">Ticket</a>
                    </li>
                    <li class="py-1">
                        <a href="{{ url('design/tour') }}">Tours</a>
                    </li>
                    <li class="py-1">
                        <a href="{{ url('design/tour') }}">Driving School </a>
                    </li>
                    <li class="py-1">
                        <a href="{{ url('design/tour') }}">Labour (श्रम)</a>
                    </li>
                </ul>
            </div>
            <div class="contact">
                <p class="text-xl underline underline-offset-2 py-2 text-white">Contact</p>
                <div class="text-sm px-2 py-1">
                    <p class="py-1">Thamel, Kathmandu, Nepal</p>
                    <p class="py-1">Tel: +977-1-4435300 , 4422325</p>
                    <p class="py-1">Cell: +977-9851093862</p>
                    <p class="py-1">Email: info@plannepal.com / plannepaltours@gmail.com</p>
                    <p class="py-1">whatapps/Viber : 9801044333</p>
                </div>

            </div>
            <div class="contact">
                <p class="text-xl underline underline-offset-2 py-2 text-white">Stay Connected</p>
                <div class="text-sm px-2 py-1">
                    <a href="#" class="py-1 block">
                        <i class="pr-2 fa-brands fa-facebook"></i>  Facebook
                    </a>
                    <a href="#" class="py-1 block">
                        <i class="pr-2 fa-brands fa-instagram"></i>  Instagram
                    </a>
                    <a href="#" class="py-1 block">
                        <i class="pr-2 fa-brands fa-twitter"></i>  Twitter
                    </a>
                </div>

            </div>
        </div>
    </div>
</footer>
<div class="bg-primary text-white h-14">
    <div class="content-container h-full">
        <div class="flex justify-between items-center h-full">
            <div>Copyright 2022, Sasto Deal Pvt. Ltd.</div>
            <div>Design & Developed By: Prajwal Poudel</div>
        </div>
    </div>
</div>
